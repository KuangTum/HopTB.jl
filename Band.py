...
jl = Julia(compiled_modules=False)  # 关键开关：禁用预编译模块，避免不兼容嵌入
...
def get_kpoints(
    ...
    with open("kpoints_weights.dat", "w") as f:
        f.write("# kx ky kz weight\n")
        for k, w in zip(kpoints, weights):
            f.write(f"{k[0]:.6f} {k[1]:.6f} {k[2]:.6f} {w:.6f}\n")
...
def calculate_fermi_level(
    ...
    print("Fermi level (eV): ", fermi_level)
...
def main(args: list = None) -> None:
    ...
    if (
        ...
        dat_file_path = os.path.join(output_dir, "fermi_level_pred_and_true.dat")
        np.savetxt(dat_file_path, np.array([fermi_level_pred, fermi_level_true]))
