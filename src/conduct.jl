...
using ..HopTB
using ..HopTB.Utilities: fermidirac, dfermi_dE, constructmeshkpts, splitkpts, construct_irreducible_kmesh
...
function AC_worker(job::Tuple{Matrix{Float64},Vector{Float64}}, tm::AbstractTBModel, α::Int64, β::Int64,
    ωs::Vector{Float64}, T::Float64, μ::Float64, δ::Float64)
    kpts, weights = job
        ...
        for (iω, ω)  in enumerate(ωs)
            for n in 1:tm.norbits
                ...
                for m in 1:tm.norbits
                    ...
                    if abs(ϵn-ϵm) > 1e-7
                        fnm = (fn-fm)/(ϵn-ϵm)
                        result[iω] += w * fnm * (vα[n, m]*vβ[m, n]) / (ϵn - ϵm + ω + im*δ) 
                    else
                        result[iω] += w * dfermi_dE(T, ϵn-μ) * (vα[n, m]*vβ[m, n] ) / ( ω + im*δ)
                    end
                end    
            end
        end
...
function _build_weighted_mesh(tm::AbstractTBModel, nkmesh::Vector{Int64}; use_symmetry::Bool=false, mesh_kwargs...)
    if use_symmetry
        kpts, weights = construct_irreducible_kmesh(tm, nkmesh; mesh_kwargs...)
        return kpts, weights
    else
...
function getAC(tm::AbstractTBModel, α::Int64, β::Int64, ωs::Vector{Float64}, nkmesh::Vector{Int64},
    T::Float64, μ::Float64; gs::Int64=1, δ::Float64=0.05, use_symmetry::Bool=true, mesh_kwargs...)
    ...
    if nkmesh[3] == 1 # 2D case
        ...
        return -im * gs * σ_ac * bzvol / (2 * pi) # e**2/hbar
    else
        return -im * gs * σ_ac * bzvol * 98.130728142 # e**2/(hbar*(2π)^3)*1.0e10/100
    end
end
